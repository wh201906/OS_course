    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(sendTimer, &QTimer::timeout, this, &MainWindow::senderProcess);
    connect(receiveTimer, &QTimer::timeout, this, &MainWindow::receiverProcess);
...
void MainWindow::on_createButton_clicked()
{
    ...
    newDataMutex = mySemaphoreOpen("/" + typeName.toLocal8Bit() + "_newDataMutex", 1);
...
void MainWindow::on_startSenderBox_clicked(bool checked)
{
    if(checked)
        sendTimer->start();
    else
...
void MainWindow::senderProcess(void)
{
    quint64 val = QRandomGenerator::global()->generate64();
    QByteArray data = QString::number(val).toLocal8Bit();
    ui->sentEdit->appendPlainText(data);
    if(isMain)
    {
        sem_wait(busyMutex);
        sharedMsg->len1 = data.length() + 1;
        memcpy(sharedMsg->data1, data.data(), sharedMsg->len1);
        sem_post(busyMutex);
    }
    else
    {
        sem_wait(newDataMutex);
        sharedMsg->len2 = data.length() + 1;
        memcpy(sharedMsg->data2, data.data(), sharedMsg->len2);
        sem_post(newDataMutex);
    }

}

void MainWindow::receiverProcess(void)
{
    quint64 num;
    if(isMain)
    {
        if(sem_trywait(newDataMutex) != 0)
            return;
        num = sharedMsg->len2;
        if(num == 0)
        {
        ...
        memcpy(buf, sharedMsg->data2, num);
        ui->receivedEdit->appendPlainText(QByteArray(buf, num));
        sharedMsg->len2 = 0;
        sem_post(newDataMutex);
    }
    else
    {
        if(sem_trywait(busyMutex) != 0)
            return;
        num = sharedMsg->len1;
        if(num == 0)
        {
        ...
        memcpy(buf, sharedMsg->data1, num);
        ui->receivedEdit->appendPlainText(QByteArray(buf, num));
        sharedMsg->len1 = 0;
        sem_post(busyMutex);
    }
...
void MainWindow::on_isMainBox_clicked(bool checked)
{
    isMain = checked;
}
